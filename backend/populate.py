...
def post(endpoint: str, data: Dict[str, Any], access_token: Optional[str] = None):
    ...
    response = requests.post(f"http://127.0.0.1:3000{endpoint}", data=json.dumps(data), headers=headers)
    if response.status_code != 200:
        fail(f"Got status code {response.status_code} from request to {endpoint}. Response body: {response.text}")
...
def follow(*, follower_access_token: str, follow_username: str) -> None:
    post("/follow", data={"follow_username": follow_username}, access_token=follower_access_token)
...
def main():
    ...
    send_bloom(musician_access_token, "New album dropping at midnight! Pre-save now! #SwizBiz")
