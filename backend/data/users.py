...
class User:
    ...
    def check_password(self, password_plaintext: str) -> bool:
        return self.password_scrypt == scrypt(password_plaintext.encode("utf-8"), self.password_salt)
...
def get_user(username: str) -> Optional[User]:
    with db_cursor() as cur:
        cur.execute(
            "SELECT id, password_salt, password_scrypt FROM users WHERE username = %s", (username,)
        )
        row = cur.fetchone()
        if row is None:
            return None
